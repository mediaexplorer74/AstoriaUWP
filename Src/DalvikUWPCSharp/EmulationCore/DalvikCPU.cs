        ...
        public void ExecuteInstruction(OpCode op, Class c)
        {
            ...
            switch(op.Instruction)
            {
                ...
                case Instructions.InvokeVirtual:
                    ...
                    try
                    {
                        Method m = dex.GetMethod(ivop.MethodIndex);
                        byte register = 0;
                        try
                        {
                            //register = ivop.ArgumentRegisters[1];
                            register = ivop.ArgumentRegisters[0];
                        }
                        catch (Exception ex1)
                        {
                            Debug.WriteLine("[ex1] Dalvik CPU - ExecuteInstruction - ivop.ArgumentRegisters[1] ex. : " + ex1.Message);
                        }
                        object obj = Registers[register];
                        RunMethod(m, c, obj);
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine("[ex] Dalvik CPU - ExecuteInstruction ex. : " + ex.Message);
                    }
                    break;
